...
_socket.bind(
    ("", 0)
)  # replace to config entries? we need it as source port has to be specified in outgoing Transport entry
...
def process_rtp(raw_rtp):
    #
    rtp_headers = rtphelper.DecodeRTPpacket(
        bytes.hex(raw_rtp)
    )  # payload is HEXed bytes
...
def playThread():
    ...
    #
    with requests.get(
        cfg.getStr("intercom_url"), headers=headers, data=raw_content, stream=True
    ) as response:
